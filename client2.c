...
int main(void){
	...
	do{
		client_sockfd=accept(sockfd, (struct sockaddr*) &client_addr,&clientlen);   //CONNECTION HANDLER
		if(client_sockfd==-1){
			perror("accept error");
		}

		char ip[INET6_ADDRSTRLEN];  // length(46) for ipv6 addresses in string form  //for ipv4=16
        inet_ntop(AF_INET, &(client_addr.sin_addr), ip, INET6_ADDRSTRLEN); //converts address from network format to character string
      
      	int port2 = (int) ntohs(client_addr.sin_port); 
      	char ip_add[strlen(ip)];
        strcpy(ip_add,ip);
